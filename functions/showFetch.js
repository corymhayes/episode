...
exports.handler = async function(event, context) {
  ...
  if (showData.results) {
    ...
    const numberOfSeasonsSearch = await fetch(`https://api.themoviedb.org/3/tv/${showId}?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`)
    ...
    const seasonSearch = await fetch(`https://api.themoviedb.org/3/tv/${showId}/season/${randomSeasonNumber}?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`)
